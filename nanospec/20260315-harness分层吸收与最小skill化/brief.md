...
- `references/repos/` 中的 `everything-claude-code`、`superpowers`、`oh-my-opencode` 都是成熟 harness，但体量大、分层复杂，部分能力带平台假设。
...
- 暂不优先吸收 hooks 自动化、深度工具集成、平台安装脚本等重运行时能力；multiagent 若只是执行路径而非平台绑定前提，可以直接吸收其 prompt 纪律。
