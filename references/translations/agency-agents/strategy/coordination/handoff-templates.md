...
> 用于 NEXUS pipeline 中各类 agent-to-agent handoff 的标准模板。稳定一致的 handoff 可以减少上下文丢失，而这正是多 agent 协作失败的第一大来源。
...
适用于 Evidence Collector 或其他 QA agent 通过某个任务时。
...
适用于 Evidence Collector 或其他 QA agent 驳回某个任务时。
