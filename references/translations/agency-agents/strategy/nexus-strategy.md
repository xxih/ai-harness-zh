...
## The Agency 的多 Agent 编排完整操作手册

> **NEXUS** 把 The Agency 原本独立运作的 AI specialists 变成一个同步协作的 intelligence network。它不是一组 prompt 的堆砌，而是一套**部署纲领**，让 The Agency 成为任意项目、产品或组织的放大器。
...
**NEXUS 通过以下方式消除这些失效模式**：

- 定义每个阶段由**谁**激活
- 明确他们要产出**什么**，交给**谁**
- 规定**何时** handoff，以及交给谁
- 规定进入下一阶段前**如何**校验质量
- 说明 pipeline 中每个 agent 存在的**原因**，避免“搭便车角色”

### 1.2 核心原则

| Principle | Description |
|-----------|-------------|
| **Pipeline Integrity** | 没有通过质量门禁，就不能进入下一阶段 |
| **Context Continuity** | 每次 handoff 都要携带完整上下文，不能让 agent 冷启动 |
| **Parallel Execution** | 独立 workstreams 并行运行，压缩整体周期 |
| **Evidence Over Claims** | 所有质量判断都必须有证据，不能只凭口头断言 |
| **Fail Fast, Fix Fast** | 单个任务最多重试 3 次，之后必须升级 |
| **Single Source of Truth** | 一个 canonical spec、一份 task list、一份 architecture doc |

### 1.3 按 Division 划分的 Agent 阵容

| Division | Agents | Primary NEXUS Role |
|----------|--------|--------------------|
| **Engineering** | Frontend Developer, Backend Architect, Mobile App Builder, AI Engineer, DevOps Automator, Rapid Prototyper, Senior Developer | 构建、部署并维护全部技术系统 |
| **Design** | UI Designer, UX Researcher, UX Architect, Brand Guardian, Visual Storyteller, Whimsy Injector, Image Prompt Engineer | 定义视觉识别、用户体验和品牌一致性 |
| **Marketing** | Growth Hacker, Content Creator, Twitter Engager, TikTok Strategist, Instagram Curator, Reddit Community Builder, App Store Optimizer, Social Media Strategist | 负责获客、互动和市场存在感 |
| **Product** | Sprint Prioritizer, Trend Researcher, Feedback Synthesizer | 定义做什么、何时做、为什么做 |
| **Project Management** | Studio Producer, Project Shepherd, Studio Operations, Experiment Tracker, Senior Project Manager | 编排时间线、资源和跨职能协作 |
...
|------|--------------|----------|----------|
| **NEXUS-Full** | All | Enterprise 产品上线、完整生命周期 | 12-24 weeks |
| **NEXUS-Sprint** | 15-25 | Feature 开发、MVP 构建 | 2-6 weeks |
| **NEXUS-Micro** | 5-10 | Bug fix、内容 campaign、单一 deliverable | 1-5 days |

---

## 3. Phase 0 — Intelligence & Discovery

> **Objective**：在投入资源前先理解外部环境。问题没有被验证前，不进入构建。
...
> **Objective**：在写下一行代码前，先定义清楚我们要构建什么、系统怎么组织、成功长什么样。
...
**Gate Keeper**：Studio Producer + Reality Checker（双签）
...
**Output**：Approved Architecture Package → 激活 Phase 2

---

## 5. Phase 2 — Foundation & Scaffolding

> **Objective**：建立后续所有工作依赖的技术与运营基础。先把骨架立起来，再谈加肌肉。
...
**Output**：Working skeleton application with full DevOps pipeline → 激活 Phase 3

---

## 6. Phase 3 — Build & Iterate

> **Objective**：通过持续的 Dev↔QA loops 构建功能。每个任务都要先通过验证，才能进入下一个任务。这是大部分工作真正发生的阶段。

### 6.1 Dev↔QA Loop

这是 NEXUS 的心脏。Agents Orchestrator 负责管理**逐任务质量循环**：
...
**Output**：Feature-complete application → 激活 Phase 4

---

## 7. Phase 4 — Quality & Hardening

> **Objective**：最终质量关卡。Reality Checker 默认给出 `NEEDS WORK`，除非你能提供压倒性的生产就绪证据。
...
**Gate Keeper**：Reality Checker（唯一裁决）
...
- **READY**：进入发布阶段（首次通过较少见）
- **NEEDS WORK**：带具体 fix list 返回 Phase 3（这是预期情况）
- **NOT READY**：存在重大架构问题，返回 Phase 1/2

**Expected**：首次实现通常需要 2-3 轮修订。B/B+ 是正常且健康的结果。

---

## 8. Phase 5 — Launch & Growth

> **Objective**：在所有渠道同时协调 go-to-market 执行。发布时获取最大影响力。
...
**Output**：Stable launched product with active growth channels → 激活 Phase 6

---

## 9. Phase 6 — Operate & Evolve

> **Objective**：在持续运营中不断改进。产品已经上线，现在要让它真正 thrive。

### 9.1 Active Agents（持续运行）
...
该矩阵说明哪些 agents 的输出会被其他 division 消费。读法是：**行是生产者，列是消费者**。
...
每一次 agent-to-agent handoff 都必须包含：
...
|----------|--------------|-------------------|--------|
| **Critical** (P0) | Immediate | Studio Producer | All-hands，停止其他工作 |
| **High** (P1) | < 4 hours | Project Shepherd | 指派专门 agent 处理 |
| **Medium** (P2) | < 24 hours | Agents Orchestrator | 提升为下一 sprint 优先事项 |
| **Low** (P3) | < 1 week | Sprint Prioritizer | 进入 backlog |
...
#### For the Orchestrator（启动 Pipeline）
...
*从 discovery 到持续运营，每个 agent 都知道自己的角色、时机和 handoff 位置。*
