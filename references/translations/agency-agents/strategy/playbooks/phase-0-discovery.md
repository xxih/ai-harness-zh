...
在正式投入资源之前先验证机会是否成立。只有在问题、市场和监管环境都被理解之后，才进入构建阶段。

## 前置条件

- [ ] 已有项目 brief 或初始概念
- [ ] 已识别 stakeholder sponsor
- [ ] discovery 阶段预算已获批准
...
六个 agents 全部提交各自报告后，由 Executive Summary Generator 做综合：
...
- **GO**：进入 Phase 1 — Strategy & Architecture
- **NO-GO**：归档 findings，记录 learnings，重定向资源
- **PIVOT**：根据 findings 调整范围或方向，并重新执行定向 discovery
...
*当 Executive Summary Generator 基于全部六个 discovery agents 的证据给出 GO 决策时，Phase 0 完成。*
